...
# makeImage(npix=500,incl=60.,phi=0.,wav=1000.,sizeau=120, nostar=True)   # This calls radmc3d 
# fig2  = plt.figure()
# a=readImage()
...
for amax_idx in [10, 1, 0.1, 0.01]:
    problem_setup(a_max=amax_idx*0.1, Mass_of_star=0.14*Msun, Accretion_rate=0.14e-5*Msun/yr, Radius_of_disk=30*au)
    ...
    fnu   = s[:,1]
    nufnu = nu*fnu
    # plt.plot(1e-9*nu,1e23*fnu,label=f"{amax_idx}mm")
    plt.plot(lam,1e23*fnu,label=f"{amax_idx}mm")
plt.xscale('log')
plt.yscale('log')
# plt.xlim((5e+1, 1e+7))
plt.xlim((1e-1, 5e+1))
plt.ylim(bottom=1e1, top=1e9)
# plt.ylim((1e-1,1e2))
# plt.xlabel('$\\nu [GHz]$')
plt.xlabel('$\lambda [mm]$')
plt.ylabel('$ Flux Density \; [mJy]$')
plt.legend()
plt.title('Dust continuum with different maximum grain size')
...
#     nufnu = nu*fnu
#     # plt.plot(1e-9*nu,1e20*fnu,label=f"{incldeg}deg")
#     plt.plot(lam,1e23*fnu,label=f"{incldeg}$^\circ$")
# # ,linestyle='dashed'
# plt.xscale('log')
# plt.yscale('log')
# # plt.xlim((1e+1, 1e+3))
...
# plt.ylabel('$ Flux Density \; [mJy]$')
# plt.legend()
# plt.title('Dust continuum with different inclination angle')
# plt.savefig('different_angle.pdf')
# # plt.show()
# plt.close()
