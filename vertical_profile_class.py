...
class DiskModel_vertical:
    ...
    def make_rho_and_m_map(self):
        ...
        for r in range(self.NR):
            ...
            """
            h_itr = np.linspace(0.01, 100, 100000)
            err = 1
            h_index = 0
    ...
    def make_tau_and_T_map(self):
        ...
        for i in range(0, np.searchsorted(self.Z_grid, 10)):
            T_500 = T_map[:, i]
            jjj = 500
            T_499 = np.where(T_500 < jjj, jjj, T_500)
            T_map[:,i]=T_499
        # self.T_500 = T_500
        self.T_map = T_map
    ...
    def extend_to_spherical(self, NTheta):
        self.NTheta = NTheta
        self.NPhi = 10 # Since this model is only axisymmetric so the value of NPhi isn't important
